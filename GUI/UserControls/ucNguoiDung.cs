...
namespace GUI.UserControls
{
    public partial class ucNguoiDung : UserControl
    {
        ...
        private void dataGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 7)
            {
                var fedit = new fEditNguoiDung(BUSNguoiDung.Instance.GetNguoiDungByMa(dataGrid.Rows[stt].Cells["MaNguoiDung"].Value.ToString()).id);
                fedit.ShowDialog();
                Bind(BUSNguoiDung.Instance.GetAllNguoiDung());
                return;
            }
            var f = new fInfoNguoiDung(BUSNguoiDung.Instance.GetNguoiDungByMa(dataGrid.Rows[stt].Cells["MaNguoiDung"].Value.ToString()).id);
            f.Show();
            Bind(BUSNguoiDung.Instance.GetAllNguoiDung());
        }
